 ...
 */
export function validateBenchmarkConfigItem(
  config: unknown,
  index: number
): string | null {
 ...
 */
export function validateBenchmarkConfigs(configs: unknown): string | null {
 ...
 */
export function createTempPath(basePath: string): string {
 ...
 */
export function padString(str: string, minLength: number, char = " "): string {
  if (str.length >= minLength) return str.slice(0, minLength);
