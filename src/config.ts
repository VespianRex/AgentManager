...
export class ConfigCache {
  private entries = new Map<string, { document: AgentManagerDocument; ts: number }>();
  private readonly ttlMs: number;
  private readonly maxEntries: number;

  // Mutex for cache operations - prevents concurrent access from corrupting state
  private _mutexQueue: (() => void)[] = [];
  ...
  async get(key: string): Promise<{ document: AgentManagerDocument; ts: number } | undefined> {
  ...
  async set(key: string, value: { document: AgentManagerDocument; ts: number } | AgentManagerDocument): Promise<void> {
    await this.acquireLock();
    try {
      // Normalize: if a raw document is passed, wrap it with current timestamp
      const entry: { document: AgentManagerDocument; ts: number } =
        "document" in value && "ts" in value
          ? (value as { document: AgentManagerDocument; ts: number })
          : { document: value as AgentManagerDocument, ts: Date.now() };
 ...
 */
const evictStaleCacheEntries = (): void => {
 ...
 * ```
 */
export const normalizePath = (filePath: string, cwd: string): string => {
  if (filePath == null) throw new Error("normalizePath: filePath cannot be null or undefined");
  if (cwd == null) throw new Error("normalizePath: cwd cannot be null or undefined");

  const sanitizedPath = String(filePath).replace(/\0/g, "");

  // Empty path resolves to cwd
  if (sanitizedPath === "") return cwd;
 ...
 * ```
 */
export const validatePathWithRealpath = async (filePath: string, expectedBase?: string): Promise<string> => {
  if (filePath == null) throw new Error("validatePathWithRealpath: filePath cannot be null or undefined");
 ...
 */
export const readJsoncFile = async (filePath: string, useCache = true): Promise<AgentManagerDocument> => {
  if (useCache) {
  ...
  try {
    ...
    let document = parse(raw) as AgentManagerDocument;
 ...
 */
export const writeJsoncFile = async (filePath: string, document: unknown): Promise<void> => {
  ...
  const merged = merge(original as Record<string, unknown>, document as Record<string, unknown>);
  ...
  if (isPlainObject(original)) {
    copyCommentSymbols(merged as Record<string, unknown>, original as Record<string, unknown>);
  }
  if (isPlainObject(document)) {
    copyCommentSymbols(merged as Record<string, unknown>, document as Record<string, unknown>);
  }
...
export const findConfigFiles = async (cwd: string): Promise<ConfigLocation[]> => {
  const discoveryLocations: ConfigLocation[] = [
    ...
    try {
      ...
      if (location.source !== "user") {
        try {
          ...
          if (isNodeError(err) && (err.code === "ELOOP")) return undefined;
        ...
        return { ...location, path: resolved } as ConfigLocation;
      }
      ...
      const baseDir =
        configPathIndex > 0 ? resolved.substring(0, configPathIndex) : process.env.HOME ?? os.homedir();
      ...
      try {
        ...
        if (isNodeError(err) && (err.code === "ENOENT" || err.code === "ELOOP")) return undefined;
        throw err;
      }
  ...
  for (const value of settled) {
    if (value !== undefined) results.push(value as ConfigLocation);
  }
  return results;
};
 ...
 */
export const backupConfig = async (filePath: string): Promise<string> => {
...
export const saveConfig = async (config: ConfigLocation, document: AgentManagerDocument): Promise<string> => {
  ...
  let validatedDocument: AgentManagerDocument;
  try {
