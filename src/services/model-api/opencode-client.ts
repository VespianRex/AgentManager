...
function estimateTokens(text: string): number {
  if (!text) return 0;
  return Math.ceil(
    text.trim().split(/\s+/).filter(Boolean).length * TOKEN_ESTIMATION.WORD_TO_TOKEN_MULTIPLIER,
  );
}

// --- Error Response Factory (KISS) ---

function createErrorResponse(elapsedMs: number, error: string): TestPromptResponse {
  return { text: "", tokensUsed: 0, finishReason: "error", elapsedMs, tokensPerSecond: 0, error };
}

// --- Response Parsers (Strategy Pattern - DRY) ---

interface ResponseParser {
  parseText(data: unknown): string;
  parseTokens(data: unknown, text: string): number;
  parseFinishReason(data: unknown): string;
}
...
const readPath = (value: unknown, path: readonly PathSegment[]): unknown => {
...
const readStringPath = (value: unknown, path: readonly PathSegment[]): string | undefined => {
  const result = readPath(value, path);
  return typeof result === "string" ? result : undefined;
};

const readNumberPath = (value: unknown, path: readonly PathSegment[]): number | undefined => {
  const result = readPath(value, path);
  return typeof result === "number" && Number.isFinite(result) ? result : undefined;
};

const openAIResponseParser: ResponseParser = {
...
const anthropicResponseParser: ResponseParser = {
...
export class OpenCodeModelApiClient {
   ...
   */
  constructor(modelId: string, timeoutMs: number = 60000, resolvedApiKey?: string) {
    ...
    if (!parser) throw new Error(`Unsupported provider: ${provider}`);
  ...
  private buildUrl(request: TestPromptRequest): string {
  ...
  private buildRequestBody(request: TestPromptRequest): Record<string, unknown> {
 ...
 * ```
 */
export function createModelApiClient(modelId: string, timeoutMs?: number): OpenCodeModelApiClient | null {
  try {
 ...
 * ```
 */
export async function createModelApiClientAsync(modelId: string, timeoutMs?: number): Promise<OpenCodeModelApiClient | null> {
  try {
    const { provider, model } = parseModelId(modelId);
...
class OpenCodeModelApiClientWithKey extends OpenCodeModelApiClient {
  constructor(modelId: string, apiKey: string, timeoutMs?: number) {
 ...
 * ```
 */
export function isModelTestable(modelId: string): boolean {
 ...
 */
export async function canTestModelAsync(modelId: string): Promise<boolean> {
  const { provider } = parseModelId(modelId);
