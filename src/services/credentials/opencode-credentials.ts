...
import { parseModelId, type CredentialReport } from "./index.js";
...
async function refreshGoogleAccessToken(refreshToken: string): Promise<{ access_token: string; expires_in: number }> {
  ...
  if (!clientId || !clientSecret) {
    throw new Error(
      `Missing OAuth credentials: GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET must be set in environment`
    );
  }
...
export async function getGoogleAccessToken(): Promise<string | undefined> {
  ...
  try {
    ...
    const accounts: AntigravityAccounts = JSON.parse(content);
...
export async function getApiKeyForProvider(provider: string): Promise<string | undefined> {
  ...
  try {
    ...
    if (config.provider && typeof config.provider === "object") {
      ...
      if (providerConfig && (providerConfig as Record<string, unknown>).apiKey) {
        return (providerConfig as Record<string, unknown>).apiKey as string;
      }
      if (providerConfig && (providerConfig as Record<string, unknown>).key) {
        return (providerConfig as Record<string, unknown>).key as string;
      }
    }
  } catch {
 ...
 */
export async function hasCredentialsForProviderAsync(provider: string): Promise<boolean> {
 ...
 */
export async function getDefaultOpenCodeModel(): Promise<{ provider: string; model: string } | undefined> {
  try {
    const content = await fs.readFile(OPENCODE_CONFIG_PATH, "utf-8");
    const config = JSON.parse(content);
 ...
 */
export async function generateCredentialReportAsync(modelId: string): Promise<CredentialReport> {
  const { provider, model } = parseModelId(modelId);
  const envVars = PROVIDER_TO_ENV_VAR[provider] || [`${provider.toUpperCase()}_API_KEY`];

  let apiKey: string | undefined;
  let source: "environment" | "oauth" | "config" | undefined;
 ...
 */
export function getCredentialStatusSync(modelId: string): {
  provider: string;
  model: string;
