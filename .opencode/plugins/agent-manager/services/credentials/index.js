 ...
 */
export function getEndpointForProvider(provider, model) {
    const config = PROVIDER_CONFIGS[provider.toLowerCase()];
    if (!config)
        return undefined;
 ...
 */
export function hasCredentialsForProvider(provider) {
 ...
 */
export function generateCredentialReport(modelId) {
    ...
    const credentialInfo = { provider, configured: !!config, envVar, hasKey: !!apiKey };
    if (!config)
        return { modelId, provider, model, credentialInfo, status: "unknown_provider", error: `Unknown provider: ${provider}` };
    if (!apiKey)
        return { modelId, provider, model, credentialInfo, status: "missing_key", endpoint, error: `No API key configured for ${provider}` };
 ...
 */
export function getCredentialStatus(modelId) {
